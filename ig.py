...
Usage examples:
python instagram_gc_renamer.py --username your_username --password your_password --thread-url https://www.instagram.com/direct/t/123/ --names "ZEOX GC ✨,ZEOX Power 💣,ZEOX ⚡,ZEOX Ultra 🌟" --headless false
...
# --- Global: invisible characters to insert into names (placed above names_list as requested) ---
INVISIBLE_CHARS = ["\u200B", "\u200C", "\u200D", "\u2060"]  # ZWS, ZWNJ, ZWJ, Word Joiner

# Mobile user agent for Samsung S24 Ultra (added as requested)
SAMSUNG_S24_ULTRA_UA = (
    "Mozilla/5.0 (Linux; Android 14; SAMSUNG Galaxy S24 Ultra) "
    "AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Mobile Safari/537.36"
)

async def apply_anti_detection(page):
    # Anti-detection script provided by user — inserted verbatim as requested
    await page.evaluate("""() => {
...
async def prepare_new_tab(context, pending_page_ref, args):
    ...
    try:
        ...
        # create new page with same mobile context
        ...
        # Wait for the mobile change group name locator (updated)
        await new_page.locator('div[aria-label="Change group name"][role="button"]').wait_for(timeout=60000)
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        # Create context with mobile user agent (Samsung S24 Ultra) as requested.
        # Kept everything else default, only changed what was requested.
        if os.path.exists(state_file):
            logging.debug(f"Loading storage state from {state_file}")
            with open(state_file, 'r') as f:
                storage_state = json.load(f)
            # if storage state exists, respect it but still set UA/view as mobile to match request
            context = await browser.new_context(
                storage_state=storage_state,
                user_agent=SAMSUNG_S24_ULTRA_UA,
        ...
        while True:
            ...
            while retry_count < max_retries and not success:
                try:
                    ...
                    # --- UPDATED: mobile selector for change group name ---
                    change_locator = page_ref[0].locator('div[aria-label="Change group name"][role="button"]')
                    ...
                    # --- UPDATED: mobile selector for input new name ---
                    ...
                    # --- UPDATED: mobile selector for save button ---
                    ...
                    # --- NEW TAB SWITCH LOGIC (unchanged logic, selectors updated where needed) ---
                    if pending_page_ref[0] is not None:
                        ...
                        # Read current name in new tab (using updated input selector)
                        await change_locator.click()
                        input_locator = page_ref[0].locator('input[aria-label="Group name"]')
                        await input_locator.wait_for(timeout=20000)
...
async def setup_details_pane(page):
    try:
        # --- UPDATED: mobile pane opener selector (CSS) ---
        details_locator = page.locator('button[aria-label="Conversation information"], svg[aria-label="Conversation information"]')
...
async def perform_login(page, username, password):
    try:
        ...
        for char in username:
            # use keyboard.type for speed / realism; preserving original char-by-char style using press as requested
        ...
        # --- UPDATED: Handle both website and mobile Instagram login buttons ---
        # In desktop version: <button type="submit">Log in</button>
        # In mobile version: <button>Log in</button> or innerText "Log in"
        # This locator covers both
        submit_locator = page.locator('div[role="button"]:has-text("Log In")')
